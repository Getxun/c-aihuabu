---
title: C-ai Agent 接入规划
description: 规划通过 C-ai Agent、MCP 和侧边栏助手连接 Codex / Claude Code 操作画布
---
...
浏览器页面不能直接启动本地进程，也不应该直接控制本机 Codex / Claude Code。推荐增加一个用户本机运行的 `c-ai-agent` 服务：

```txt
线上画布网页 <-> 本机 c-ai-agent <-> Codex / Claude Code
                         |
                         +-> MCP Server
                         +-> 画布连接
```

`c-ai-agent` 是本地可信边界，负责启动或连接 Codex / Claude Code、暴露 MCP 工具、维护画布连接、转发侧边栏对话事件。

## 源码目录

后续实现时建议在项目根目录新增独立 npm 包；源码目录暂用 `canvas-agent/`，对外包名和命令统一为 `c-ai-agent`：
...
前端不直接暴露 HTTP 接口给本机服务，优先由网页主动连接本机 C-ai Agent。当前 MVP 使用 SSE 接收本机事件，HTTP POST 上报画布状态和工具结果：
...
`c-ai-agent` 内置 MCP Server，让 Codex CLI 可以连接：

```txt
Codex CLI <-> c-ai-agent MCP <-> 当前网页画布
```
...
网页侧边栏助手另走 C-ai Agent 的对话通道。C-ai Agent 使用官方 `@openai/codex` CLI 的 `codex app-server --stdio` 启动并续用同一个 thread，启动时会注入 `infinite-canvas` MCP 配置并把 `default_tools_approval_mode` 设为 `approve`，避免 Codex 自己的 MCP 审批卡住侧边栏；真正修改画布前由网页侧边栏做二次确认。

侧边栏图片附件通过 HTTP 发送到本机 C-ai Agent，C-ai Agent 临时写入本机文件后作为 app-server `localImage` 输入传给 Codex；MVP 会在输入区提示附件体积，单次请求体限制为 30MB。

## 侧边栏助手设计

侧边栏助手连接 `c-ai-agent` 后，由 C-ai Agent 选择 Agent Adapter：

```txt
Sidebar -> c-ai-agent -> Codex app-server stdio
Sidebar -> c-ai-agent -> Claude Code CLI / Claude Agent SDK
```
...
- Codex app-server 会输出 `item/agentMessage/delta`；C-ai Agent 转成 `item.updated` 后，前端用同一条消息做真实流式渲染。
...
Claude Code 侧当前 MVP 先调用本机 Claude Code CLI 的流式 JSON 输出，由 `c-ai-agent` 统一转换事件和工具调用；后续可升级为 Claude Agent SDK。
...
C-ai Agent 调用 Claude Code 时默认允许 `mcp__infinite-canvas__*`，避免 print 模式被工具审批卡住；真正修改画布仍由网页侧边栏确认。
...
- C-ai Agent 默认只监听 `127.0.0.1`。
- C-ai Agent 启动时生成 token，网页连接必须携带 token。
- C-ai Agent 只接受允许的 Origin，默认允许用户当前打开的画布域名。
- 画布操作默认先在侧边栏展示工具调用，`canvas_apply_ops` 默认需要用户确认后执行，并保留最近一次工具操作撤销入口。
- 不把 Codex / Claude 登录态、API Key、用户本地文件路径上传到线上服务。
- 线上网页只保存 C-ai Agent 地址和必要偏好，不保存本地密钥。

## 实现阶段

### 第一阶段：Codex 终端操作画布

1. 新增 `canvas-agent/` 源码目录和 `c-ai-agent` npm 包。
2. C-ai Agent 提供 SSE/HTTP 连接，网页连接并注册当前画布。
3. 前端新增 `canvasAgent` 控制层。
4. C-ai Agent 内置 MCP Server。
5. Codex 通过 MCP 调用 `canvas_get_state` 和 `canvas_apply_ops`。

### 第二阶段：网页侧边栏连接 Codex

1. 前端新增 C-ai Agent 侧边栏。
2. C-ai Agent 通过官方 `@openai/codex` CLI 的 `codex app-server --stdio` 接入 Codex，复用同一个 thread，注入 `infinite-canvas` MCP，并展示结构化事件流。
3. 运行日志只保留关键 Codex 事件，避免把流式中间更新刷满日志。
4. 侧边栏展示消息流、工具调用、工具结果、错误和图片附件大小提示。

### 第三阶段：接入 Claude Code

1. C-ai Agent 新增 Claude Code CLI Adapter。
...
- 用户运行 `npx -y c-ai-agent` 后，线上画布能显示已连接。
...
- 断开 C-ai Agent 后，网页能显示清晰的本机连接失败提示。
