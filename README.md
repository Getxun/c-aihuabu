...
| **AI 创作** | 前台直连 OpenAI 兼容接口，支持文生图、图生图、参考图编辑、文本问答、音视频生成；Seedance 2.0 可经火山方舟 Agent Plan 接入 |
| **画布助手** | 围绕选中节点与上游节点对话、生图，并把结果插回画布 |
| **本地 Agent** | 通过本机 Canvas Agent 连接 Codex / Claude Code，让 Agent 经 MCP 操作当前画布 |
| **提示词库** | 抓取多个 GitHub 开源项目的提示词，并缓存在运行实例内存中 |
...
<table>
  <tr>
    <td><b>前端</b></td>
    <td>Next.js · React · TypeScript · Tailwind CSS · Ant Design · Zustand · TanStack Query</td>
  </tr>
  <tr>
    <td><b>服务端</b></td>
    <td>少量 Next.js Route：第三方提示词内存缓存、WebDAV 可选代理</td>
  </tr>
  <tr>
    <td><b>部署</b></td>
    <td>Vercel · Docker</td>
...
推荐直接导入仓库到 Vercel，根目录已提供 `vercel.json`，会自动构建 `web/`。AI API Key、Base URL、画布、素材与生成记录默认保存在浏览器本地。
...
启动后访问 `http://localhost:3000`，进入右上角配置，填入自己的 OpenAI 兼容 `Base URL` 与 `API Key` 即可开始创作。
...
<table>
  ...
  <tr>
    ...
    <td>· <a href="canvas-agent/README.md">本地 Canvas Agent</a></td>
  </tr>
</table>
